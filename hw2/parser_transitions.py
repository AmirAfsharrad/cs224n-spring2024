...
def minibatch_parse(sentences, model, batch_size):
    ...
    dependencies = []
    all_parses = [PartialParse(sentence) for sentence in sentences]
    partial_parses = all_parses[:]

    while len(partial_parses) > 0:
        partial_parses_minibatch = partial_parses[:batch_size]
        ...
        for parse, transition in zip(partial_parses_minibatch, transitions):
            ...
            if len(parse.buffer) == 0 and len(parse.stack) == 1:
                partial_parses.remove(parse)
    
    dependencies = [parse.dependencies for parse in all_parses]
    return dependencies
